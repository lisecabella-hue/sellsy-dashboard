...
async function getAccessToken() {
  ...
  console.log('CLIENT_ID length:', process.env.SELLSY_CLIENT_ID?.length);
  console.log('CLIENT_SECRET length:', process.env.SELLSY_CLIENT_SECRET?.length);
  console.log('Body preview:', params.toString().slice(0, 60) + '...');
  ...
  console.log('Auth response body:', text.slice(0, 300));
...
async function getTotalCA(token, dateStart, dateEnd) {
  ...
  let hasMore = true;
  while (hasMore) {
    const params = new URLSearchParams({ 'limit': '100', 'offset': String(offset), 'field[]': 'amounts.total_excl_tax' });
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/cache-status') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ hasCache: !!memCache, cacheTime, months: memCache ? Object.keys(memCache).length : 0 }));
    return;
  }
